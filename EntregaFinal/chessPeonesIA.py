import random
import pygame,sys
import time


board = [['  ' for i in range(8)] for i in range(8)] ##crea el tablero


icon = pygame.image.load('b_king.png') #icono app

## Clase general para todas las fichas
class Piece:
    def __init__(self, team, type, image,value, killable=False):
        self.team = team
        self.type = type
        self.killable = killable
        self.image = image
        self.value = value


## crear las fichas: pawn, king, rook and bishop
## el primer parametro es equipo, el segundo es tipo
bp = Piece('b', 'p', 'b_pawn.png',1)
wp = Piece('w', 'p', 'w_pawn.png',1)
bk = Piece('b', 'k', 'b_king.png',100)
wk = Piece('w', 'k', 'w_king.png',100)


##inicia la parte visual de las fichas
starting_order = {(0, 0): None,  (1, 0): None,(2, 0): None, (3, 0): None,
                  (4, 0): pygame.image.load(bk.image),(5, 0): None, (6, 0): None, (7, 0): None,

                  (0, 1): pygame.image.load(bp.image), (1, 1): pygame.image.load(bp.image),
                  (2, 1): pygame.image.load(bp.image), (3, 1): pygame.image.load(bp.image),
                  (4, 1): pygame.image.load(bp.image), (5, 1): pygame.image.load(bp.image),
                  (6, 1): pygame.image.load(bp.image), (7, 1): pygame.image.load(bp.image),

                  (0, 2): None, (1, 2): None, (2, 2): None, (3, 2): None,
                  (4, 2): None, (5, 2): None, (6, 2): None, (7, 2): None,
                  (0, 3): None, (1, 3): None, (2, 3): None, (3, 3): None,
                  (4, 3): None, (5, 3): None, (6, 3): None, (7, 3): None,
                  (0, 4): None, (1, 4): None, (2, 4): None, (3, 4): None,
                  (4, 4): None, (5, 4): None, (6, 4): None, (7, 4): None,
                  (0, 5): None, (1, 5): None, (2, 5): None, (3, 5): None,
                  (4, 5): None, (5, 5): None, (6, 5): None, (7, 5): None,

                  (0, 6): pygame.image.load(wp.image), (1, 6): pygame.image.load(wp.image),
                  (2, 6): pygame.image.load(wp.image), (3, 6): pygame.image.load(wp.image),
                  (4, 6): pygame.image.load(wp.image), (5, 6): pygame.image.load(wp.image),
                  (6, 6): pygame.image.load(wp.image), (7, 6): pygame.image.load(wp.image),
                  (0, 7): None, (1, 7): None,(2, 7): None, (3, 7): None,
                  (4, 7): pygame.image.load(wk.image), (5, 7): None,(6, 7): None, (7, 7): None,}



def create_board(board):
    board[0][4] = Piece('b', 'k', 'b_king.png',100)

    board[7][4] =  Piece('w', 'k', 'w_king.png',100)

    for i in range(8):
        board[1][i] = Piece('b', 'p', 'b_pawn.png',1)
        board[6][i] = Piece('w', 'p', 'w_pawn.png',1)
    return board


## returns the input if the input is within the boundaries of the board
def on_board(position):
    if position[0] > -1 and position[1] > -1 and position[0] < 8 and position[1] < 8:
        return True


## returns a string that places the rows and columns of the board in a readable manner
def convert_to_readable(board):
    output = ''

    for i in board:
        for j in i:
            try:
                output += j.team + j.type + ', '
            except:
                output += j + ', '
        output += '\n'
    return output


## resets "x's" and killable pieces
def deselect():
    for row in range(len(board)):
        for column in range(len(board[0])):
            if board[row][column] == 'x ':
                board[row][column] = '  '
            else:
                try:
                    board[row][column].killable = False
                except:
                    pass
    return convert_to_readable(board)


## Takes in board as argument then returns 2d array containing positions of valid moves
def highlight(board):
    highlighted = []
    for i in range(len(board)):
        for j in range(len(board[0])):
            if board[i][j] == 'x ':
                highlighted.append((i, j))
            else:
                try:
                    if board[i][j].killable:
                        highlighted.append((i, j))
                except:
                    pass
    return highlighted

def check_team(moves, index):
    row, col = index
    if moves%2 == 0:
        if board[row][col].team == 'w':
            return True
    else:
        if board[row][col].team == 'b':
            return True

## This takes in a piece object and its index then runs then checks where that piece can move using separately defined functions for each type of piece.
def select_moves(piece, index, moves):
    if check_team(moves, index):
        if piece.type == 'p':
            if piece.team == 'b':
                return highlight(pawn_moves_b(index))
            else:
                return highlight(pawn_moves_w(index))

        if piece.type == 'k':
            return highlight(king_moves(index))


## Basically, check black and white pawns separately and checks the square above them. If its free that space gets an "x" and if it is occupied by a piece of the opposite team then that piece becomes killable.
def pawn_moves_b(index):
    if index[0] == 1:
        if board[index[0] + 2][index[1]] == '  ' and board[index[0] + 1][index[1]] == '  ':
            board[index[0] + 2][index[1]] = 'x '
    bottom3 = [[index[0] + 1, index[1] + i] for i in range(-1, 2)]

    for positions in bottom3:
        if on_board(positions):
            if bottom3.index(positions) % 2 == 0:
                try:
                    if board[positions[0]][positions[1]].team != 'b':
                        board[positions[0]][positions[1]].killable = True
                except:
                    pass
            else:
                if board[positions[0]][positions[1]] == '  ':
                    board[positions[0]][positions[1]] = 'x '
    return board

def pawn_moves_w(index):
    if index[0] == 6:
        if board[index[0] - 2][index[1]] == '  ' and board[index[0] - 1][index[1]] == '  ':
            board[index[0] - 2][index[1]] = 'x '
    top3 = [[index[0] - 1, index[1] + i] for i in range(-1, 2)]

    for positions in top3:
        if on_board(positions):
            if top3.index(positions) % 2 == 0:
                try:
                    if board[positions[0]][positions[1]].team != 'w':
                        board[positions[0]][positions[1]].killable = True
                except:
                    pass
            else:
                if board[positions[0]][positions[1]] == '  ':
                    board[positions[0]][positions[1]] = 'x '
    return board


## This just checks a 3x3 tile surrounding the king. Empty spots get an "x" and pieces of the opposite team become killable.
def king_moves(index):
    for y in range(3):
        for x in range(3):
            if on_board((index[0] - 1 + y, index[1] - 1 + x)):
                if board[index[0] - 1 + y][index[1] - 1 + x] == '  ':
                    board[index[0] - 1 + y][index[1] - 1 + x] = 'x '
                else:
                    if board[index[0] - 1 + y][index[1] - 1 + x].team != board[index[0]][index[1]].team:
                        board[index[0] - 1 + y][index[1] - 1 + x].killable = True
    return board



WIDTH = 600
WIDTH2= 500

WIN = pygame.display.set_mode((WIDTH, WIDTH))

""" This is creating the window that we are playing on, it takes a tuple argument which is the dimensions of the window so in this case 800 x 800px
"""


WHITE = (220, 220, 220)
GREY = (128, 128, 128)
YELLOW = (204, 204, 0)
BLUE = (50, 255, 255)
BLACK = (0, 0, 0)
GREEN = (0, 200, 0)


class Node:
    def __init__(self, row, col, width):
        self.row = row
        self.col = col
        self.x = int(row * width)
        self.y = int(col * width)
        self.colour = WHITE
        self.occupied = None

    def draw(self, WIN):
        pygame.draw.rect(WIN, self.colour, (self.x, self.y, WIDTH2 / 8, WIDTH2 / 8))

    def setup(self, WIN):
        if starting_order[(self.row, self.col)]:
            if starting_order[(self.row, self.col)] == None:
                pass
            else:
                WIN.blit(starting_order[(self.row, self.col)], (self.x, self.y))

        """
        For now it is drawing a rectangle but eventually we are going to need it
        to use blit to draw the chess pieces instead
        """


def make_grid(rows, width):
    grid = []
    gap = width // rows
    for i in range(rows):
        grid.append([])
        for j in range(rows):
            node = Node(j, i, gap)
            grid[i].append(node)
            if (i+j)%2 ==1:
                grid[i][j].colour = GREY
    return grid
"""
This is creating the nodes thats are on the board(so the chess tiles)
I've put them into a 2d array which is identical to the dimesions of the chessboard
"""


def draw_grid(win, rows, width):
    gap = width // 8
    for i in range(rows):
        pygame.draw.line(win, BLACK, (0, i * gap), (width, i * gap))
        for j in range(rows):
            pygame.draw.line(win, BLACK, (j * gap, 0), (j * gap, width))

    """
    The nodes are all white so this we need to draw the grey lines that separate all the chess tiles
    from each other and that is what this function does"""


def update_display(win, grid, rows, width):
    for row in grid:
        for spot in row:
            spot.draw(win)
            spot.setup(win)
    draw_grid(win, rows, width)
    pygame.display.update()


def Find_Node(pos, WIDTH):
    interval = WIDTH / 8
    y, x = pos
    rows = y // interval
    columns = x // interval
    return int(rows), int(columns)


def display_potential_moves(positions, grid):
    for i in positions:
        x, y = i
        grid[x][y].colour = YELLOW
        """
        Displays all the potential moves
        """


def Do_Move(OriginalPos, FinalPosition, WIN):
    starting_order[FinalPosition] = starting_order[OriginalPos]
    starting_order[OriginalPos] = None


def remove_highlight(grid):
    for i in range(len(grid)):
        for j in range(len(grid[0])):
            if (i+j)%2 == 0:
                grid[i][j].colour = WHITE
            else:
                grid[i][j].colour = GREY
    return grid
"""this takes in 2 co-ordinate parameters which you can get as the position of the piece and then the position of the node it is moving to
you can get those co-ordinates using my old function for swap"""
def check_State(board):
    score=0
    for i in range(8):
        for j in range(8):
            if type(board[i][j]) == Piece:
                piece = board[i][j]
                if (getattr(piece, 'team')) == 'w':
                    score=score+getattr(piece, 'value')
                elif(getattr(piece, 'team')) == 'b':
                    score=score-getattr(piece, 'value')
    return score



#def moveIA(board):
#    Move=findBestMove(board)
#    deselect()
#    board[Move[0]][Move[1]] = board[Move[2]][Move[3]]
#    board[Move[2]][Move[3]] = '  '

#    return board, Move



def movAleatorio(board):
    found=True
    possible = None
    moveBest = None
    while(found):
        i = random.randint(0, 7)
        j = random.randint(0, 7)
        if type(board[i][j]) == Piece:
            piece = board[i][j]
            if (getattr(piece, 'team')) == 'b':
                deselect()
                possible = select_moves((board[i][j]), (i, j), 1)
                if possible != []:
                    random.shuffle(possible)
                    p=possible[0]
                    moveBest = [p[0], p[1], i, j]
                    found=False
                else:
                    found = True
    return moveBest


def findBestMove(board):
    maxScore=100
    moveBest=None
    boardState=check_State(board)
    for i in range(8):
        for j in range(8):
            if type(board[i][j]) == Piece:
                piece = board[i][j]
                if (getattr(piece, 'team')) == 'b':
                    possible=select_moves((board[i][j]), (i, j), 1)
                    for positions in possible:
                        row, col = positions
                        if row == 7:
                            aux=board[row][col]
                            board[row][col] = board[i][j]
                            board[i][j] = '  '
                            deselect()
                            score = -100
                        else:
                            aux = board[row][col]
                            board[row][col] = board[i][j]
                            board[i][j] = '  '
                            deselect()
                            score=check_State(board)
                        if score<maxScore:
                            maxScore=score
                            moveBest=[row, col, i, j]
                        board[i][j] = board[row][col]
                        board[row][col] = aux
    if maxScore == boardState:
        return movAleatorio(board)
    else:
        return moveBest





def check_victoryW(board):
    victory=False
    kingEnemy=False

    for i in range(1):
        for j in range(8):
            if type(board[i][j]) == Piece:
                piece = board[i][j]
                if (getattr(piece, 'team')) == 'w' and (getattr(piece, 'type')) == 'p':
                    victory = True

    for i in range(8):
        for j in range(8):
            if type(board[i][j]) == Piece:
                piece = board[i][j]
                if (getattr(piece, 'team')) == 'b' and (getattr(piece, 'type')) == 'k':
                    kingEnemy = True

    if kingEnemy==False:
        victory=True


    return victory

def check_victoryB(board):
    victory=False
    kingEnemy=False


    for j in range(8):
        if type(board[7][j]) == Piece:
            piece = board[7][j]
            if (getattr(piece, 'team')) == 'b' and (getattr(piece, 'type')) == 'p':
                victory = True

    for i in range(8):
        for j in range(8):
            if type(board[i][j]) == Piece:
                piece = board[i][j]
                if (getattr(piece, 'team')) == 'w' and (getattr(piece, 'type')) == 'k':
                    kingEnemy = True

    if kingEnemy==False:
        victory=True


    return victory

def check_victory(board):
    if check_victoryW(board)==True:
        return 1
    if check_victoryB(board)==True:
        return 2
    else:
        return 0

##def movimientoIA(board):





def draw_text1(text, font, color, surface, x, y):
    textobj = font.render(text, 1, color)
    textrect = textobj.get_rect()
    textrect.topleft = (x, y)
    surface.blit(textobj, textrect)


def restart_board():
    global board
    board=[['  ' for i in range(8)] for i in range(8)]
    create_board(board)
    global starting_order
    starting_order = {(0, 0): None, (1, 0): None, (2, 0): None, (3, 0): None,
                      (4, 0): pygame.image.load(bk.image), (5, 0): None, (6, 0): None, (7, 0): None,

                      (0, 1): pygame.image.load(bp.image), (1, 1): pygame.image.load(bp.image),
                      (2, 1): pygame.image.load(bp.image), (3, 1): pygame.image.load(bp.image),
                      (4, 1): pygame.image.load(bp.image), (5, 1): pygame.image.load(bp.image),
                      (6, 1): pygame.image.load(bp.image), (7, 1): pygame.image.load(bp.image),

                      (0, 2): None, (1, 2): None, (2, 2): None, (3, 2): None,
                      (4, 2): None, (5, 2): None, (6, 2): None, (7, 2): None,
                      (0, 3): None, (1, 3): None, (2, 3): None, (3, 3): None,
                      (4, 3): None, (5, 3): None, (6, 3): None, (7, 3): None,
                      (0, 4): None, (1, 4): None, (2, 4): None, (3, 4): None,
                      (4, 4): None, (5, 4): None, (6, 4): None, (7, 4): None,
                      (0, 5): None, (1, 5): None, (2, 5): None, (3, 5): None,
                      (4, 5): None, (5, 5): None, (6, 5): None, (7, 5): None,

                      (0, 6): pygame.image.load(wp.image), (1, 6): pygame.image.load(wp.image),
                      (2, 6): pygame.image.load(wp.image), (3, 6): pygame.image.load(wp.image),
                      (4, 6): pygame.image.load(wp.image), (5, 6): pygame.image.load(wp.image),
                      (6, 6): pygame.image.load(wp.image), (7, 6): pygame.image.load(wp.image),
                      (0, 7): None, (1, 7): None, (2, 7): None, (3, 7): None,
                      (4, 7): pygame.image.load(wk.image), (5, 7): None, (6, 7): None, (7, 7): None, }


create_board(board)


def chess(WIN, WIDTH):
    imgkb = pygame.image.load('b_king1.png')
    imgkb = pygame.transform.scale(imgkb, (40, 40))
    imgkw = pygame.image.load('w_king1.png')
    imgkw = pygame.transform.scale(imgkw, (40, 40))

    moves = 0
    selected = False
    piece_to_move=[]
    grid = make_grid(8, WIDTH2)
    font2 = pygame.font.SysFont('Arial', 30)
    font22 = pygame.font.SysFont('Arial', 25)
    fontNum = pygame.font.SysFont('Arial', 100)
    buttonRegreso = pygame.Rect(160, 550, 280, 50)

    cuadroModo = pygame.Rect(0, 570, 120, 40)
    bordeTablero = pygame.Rect(0, 0, 500, 500)
    contadorP2 = 0
    contadorP1 = 0


    while True:

        WIN.fill((40, 40, 40))
        pygame.draw.rect(WIN, (20, 20, 20), buttonRegreso)
        pygame.draw.rect(WIN, (0, 0, 0), buttonRegreso, 5)

        draw_text1(str(contadorP2), fontNum, (0, 0, 0), WIN, 530, 150)
        WIN.blit(imgkb, (550, 230))
        draw_text1(str(contadorP1), fontNum, (255, 255, 255), WIN, 530, 280)
        WIN.blit(imgkw, (550, 360))

        pygame.draw.rect(WIN, (194, 81, 0), cuadroModo)
        pygame.draw.rect(WIN, (255, 69, 10), cuadroModo, 5)
        draw_text1('V.S IA', font22, (200, 200, 200), WIN, 25, 570)

        pygame.draw.rect(WIN, (0, 0, 0), bordeTablero, 10)

        draw_text1('Regresar al menú', font2, (200, 200, 200), WIN, 195, 555)
        pygame.time.delay(50) ##stops cpu dying
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
                sys.exit()

            """This quits the program if the player closes the window"""
            if event.type == pygame.MOUSEBUTTONDOWN:
                pos = pygame.mouse.get_pos()
                if buttonRegreso.collidepoint(pos):
                    restart_board()
                    import main
                    main.startMenu()

            if event.type == pygame.MOUSEBUTTONDOWN:
                pos = pygame.mouse.get_pos()
                y, x = Find_Node(pos, WIDTH2)

                if selected == False:
                    try:
                        possible = select_moves((board[x][y]), (x,y), moves)
                        for positions in possible:
                            row, col = positions
                            grid[row][col].colour = GREEN
                        piece_to_move = x,y
                        selected = True
                    except:
                        piece_to_move = []
                    #print(piece_to_move)

                else:
                    try:
                        if board[x][y].killable == True:
                            row, col = piece_to_move ## coords of original piece
                            board[x][y] = board[row][col]
                            board[row][col] = '  '
                            deselect()
                            remove_highlight(grid)
                            Do_Move((col, row), (y, x), WIN)
                            moves += 1
                            if (check_victory(board))==1:
                                contadorP1=contadorP1+1
                                moves=0
                                restart_board()
                            if moves%2==1:
                                Move = findBestMove(board)
                                deselect()
                                board[Move[0]][Move[1]] = board[Move[2]][Move[3]]
                                board[Move[2]][Move[3]] = '  '
                                Do_Move((Move[3], Move[2]), (Move[1], Move[0]), WIN)
                                moves += 1
                            if (check_victory(board))==2:
                                moves=0
                                contadorP2=contadorP2+1
                                restart_board()


                        else:
                            deselect()
                            remove_highlight(grid)
                            selected = False

                    except:
                        if board[x][y] == 'x ':
                            row, col = piece_to_move
                            board[x][y] = board[row][col]
                            board[row][col] = '  '
                            deselect()
                            remove_highlight(grid)
                            Do_Move((col, row), (y, x), WIN)
                            moves += 1
                            if (check_victory(board))==1:
                                moves=0
                                contadorP1=contadorP1+1
                                restart_board()
                            if moves%2==1:
                                Move = findBestMove(board)
                                deselect()
                                board[Move[0]][Move[1]] = board[Move[2]][Move[3]]
                                board[Move[2]][Move[3]] = '  '
                                Do_Move((Move[3], Move[2]), (Move[1], Move[0]), WIN)
                                moves +=1
                            if (check_victory(board))==2:
                                moves=0
                                contadorP2=contadorP2+1
                                restart_board()

                        else:
                            deselect()
                            remove_highlight(grid)
                            selected = False
                    selected = False

        update_display(WIN, grid, 8, WIDTH2)

